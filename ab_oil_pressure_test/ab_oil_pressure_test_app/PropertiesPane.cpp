...
const TCHAR* const CPropertiesPane::to_string( Property category)
{
	switch( category )
	{
	...
	case Property::ticket_property04: return _T("Ticket Property: Test Data" );
	case Property::ticket_property05: return _T("Ticket Property: Remark" );
	default: return _T( "" );
	}
}
...
int CPropertiesPane::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_wndObjectCombo.Create(dwViewStyle, rectDummy, this, IDC_COMBO_OBJECT ))
	{
	...
	{
		int index{ m_wndObjectCombo.AddString( to_string( category ) ) };
		m_wndObjectCombo.SetItemData( index, static_cast<DWORD_PTR>(category) );
	}
...
void CPropertiesPane::OnCbnSelchangeComboObject()
{
	int index{ m_wndObjectCombo.GetCurSel() };
	Property category{ static_cast<Property>(m_wndObjectCombo.GetItemData( index )) };
}
